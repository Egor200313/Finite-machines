...
from src.Tree import RegexParseTree
import copy
...
def test_include_word():
    regex = "ab+c.aba.*.bac.+.+*"
    regParseTree = RegexParseTree(regex)
    machine = regParseTree.create_machine()

    word = "abac"
    assert machine.has_word(word) is True
